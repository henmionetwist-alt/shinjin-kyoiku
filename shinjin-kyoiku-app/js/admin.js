    ...
    cards.push(`<div class="card">
      ...
      ${pend.map(i => `<div class="row">
        <div class="row-main"><span class="badge badge-type">${TYPE_LABELS[i.type] || ''}</span> ${esc(i.title)}
          <div class="muted small">${fmtDateTime(progressMap[e.id][i.id])} に履修</div></div>
...
function renderEmpDetail() {
  ...
  const itemsHtml = groupItems(pub).map(g => `<h3 class="section-title">${esc(g.name)}</h3>${g.items.map(i => {
...
function renderItems() {
  ...
  wrap.innerHTML = groupItems(items).map(g => `<h3 class="section-title">${esc(g.name)}</h3>${g.items.map(i => `
...
function openItemModal(item) {
  const isNew = !item;
  const it = item || { type: 'check', group: '', title: '', description: '', videoUrl: '', published: true };
  openModal(`<h3>${isNew ? '項目を追加' : '項目を編集'}</h3>
    <form id="item-form">
      ...
      <label>グループ（任意）
        <input id="it-group" list="it-group-list" value="${esc(it.group || '')}" placeholder="例：初日 / 接客 / システム操作">
...
function parseBulk(text, type) {
  return text.split(/\r?\n/).map(l => l.trim()).filter(Boolean).map(line => {
    ...
    return { title, description, videoUrl, type: t };
  }).filter(x => x.title);
}

function openBulkModal() {
  openModal(`<h3>まとめ登録</h3>
    <div class="hint">1行に1項目。「題名｜説明」の形で書きます。<br>説明がなければ題名だけ、ビデオは「題名｜説明｜URL」。<br>区切りは ｜ | ➡ → タブ のどれでも使えます</div>
    <form id="bulk-form">
      <label>種類
        <select id="bk-type">
      ...
      <label>グループ（任意・全行に付きます）
        <input id="bk-group" list="bk-group-list" placeholder="例：初日">
  ...
  const preview = () => {
    const rows = parseBulk($('#bk-text').value, $('#bk-type').value);
    $('#bk-preview').textContent = rows.length ? `${rows.length} 件を登録します（チェック ${rows.filter(r => r.type === 'check').length}・説明あり ${rows.filter(r => r.type === 'text').length}・ビデオ ${rows.filter(r => r.type === 'video').length}）` : '';
  };
    ...
    try {
      ...
      for (let i = 0; i < rows.length; i += 400) {
        ...
        for (const r of rows.slice(i, i + 400)) {
          batch.set(db.collection('items').doc(), { ...r, group, order: order++, published: true, createdAt: FV.serverTimestamp() });
        }
        await batch.commit();
      }
      closeModal();
